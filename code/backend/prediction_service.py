...
index = faiss.read_index("AI_scan/index_joconde2.faiss")
...
def get_link_with_id(id):
    try:
        with open("AI_scan/link_id2.txt", "r", encoding="utf-8") as f:
            lines = f.readlines()
            index = int(id) 
...
def find_most_similar_image(image, index, model, device, k=1, threshold=0.8):
    results = []
